                        ...
                        <Flex>
                            <Flex vertical flex={7}>
                                ...
                                <Typography.Link href={item.doi_url} target="_blank" rel="noopener noreferrer">
                                    <Typography.Title level={4} className="titleText">
                                        {item.title}
                                    </Typography.Title>
                                </Typography.Link>
...
const AuthorList: React.FC<{ authors: Author[] }> = ({ authors }) => {
    ...
    const DotIcon = (props: Partial<CustomIconComponentProps>) => <Icon component={DotSvg} {...props} />
    ...
    return (
        <Flex style={{ width: "100%", justifyContent: "space-around" }}>
            <ScopusProfileRedirectButton
                author_firstname={author.given_name}
                author_lastname={author.surname}
                url={author.scopus_url}
            />
            <DblpProfileRedirectButton authorName={authorName} />
            <ScholarProfileRedirectButton authorName={authorName} url={author.scholar_url} />
        </Flex>
    )
}
